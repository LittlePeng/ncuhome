...
{
  public  class GuestBook_Head:SkinnedBlogWebControl
    {
        public GuestBook_Head()
        {
            SkinFileName = "GuestBook_Head.ascx";
            IsThemed = true;
        }
        protected override void OnInit(EventArgs e)
        {
            //只有space 有Guestbook
            if (!Ncuhome.Blog.Core.BlogContext.Current.ThemeName.Trim().Contains("space"))
            {
                Context.Server.Transfer("userinfor.aspx?mfiid=" + blogContext.MFiiD.ToString());
            }
            base.OnInit(e);
        }
